...
conn.execute "delete * from sosoon_note where id=" & request("zid")
response.Redirect("sosoon_note_view.asp?option_id_del=succ")
end if
%>
<%
...
conn.Execute("update sosoon_note set subject='"&subject&"', memo='"&memo&"' where id=" & request("zid"))
response.Write("<SCRIPT LANGUAGE=JavaScript> alert('编辑成功');</SCRIPT>")
end if
%>
<%
sql="select * from sosoon_note where id="&request("zid") 
...
<!--                                                   ERROR MSGS
<!-------------------------------------------------------------------------------------------------------------------- -->
<FORM action="sosoon_note_edit.asp" method="post" name="form_administrator" id="form_administrator" onSubmit="return checkform();">
