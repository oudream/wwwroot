...
if rs("adminlevel")=9 then
usql="select * from dbm_guest where guest_id="&rs("user_id") 
else
...
%> 
    ...
    <td align="center" bgcolor="#FFFFFF">&nbsp; 
<%
if rs("user_id")=session("user_id") or session("user_adminlevel")=1 then
%>	
	<a href="dbm_note_edit.asp?zid=<%=rs("id")%>">编辑</a> | <a href="dbm_note_view.asp?zid=<%=rs("id")%>&option=del"  onClick="return confirm('确定删除此项吗？\n \n 将删除此留言')">删除</a>
<%end if%>
	</td>
  </tr>
  <%
