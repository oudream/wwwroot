...
if nric="" then nric=" "
if uname<>"" then
if adminlevel=1 or adminlevel=3 then sql="insert into player (name,contact,email,uid,pwd,nric,adminlevel,inserttime) values ('"&uname&"','"&contact&"','"&email&"','"&uid&"','"&pwd&"','"&nric&"',"&adminlevel&",'"&now()&"')"
if adminlevel=2 then sql="insert into player (name,contact,email,uid,pwd,nric,adminlevel,inserttime) values ('"&uname&"','"&contact&"','"&email&"','"&uid&"','"&pwd&"','"&nric&"',"&adminlevel&",'"&now()&"')"
conn.Execute(sql)
response.Write("<SCRIPT LANGUAGE=JavaScript> alert(' Create administrator is complete. ');</SCRIPT>")
end if
url="user_v.asp?adminlevel=" & adminlevel
...
<!------------------------------------------------------------------------------------------------------------------------------------ -->
        <TD width=193 height=45> <DIV align=center><FONT 
                        ...
                        color=#000000 size=3>Administrator level</FONT></DIV></TD>
        <TD width=307>
<%
if adminlevel="" then
%>
          <select name="adminlevel" id="adminlevel" class="display_dropdown">
...
if adminlevel=1 then response.Write("adminstrator")
if adminlevel=2 then response.Write("manager")
if adminlevel=3 then response.Write("visitor")
end if
%>
        </TD>
      </TR>
