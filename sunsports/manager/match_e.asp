...
%>
          ...
          <tr> 
            ...
            <td width="75%"><select name="htid" class="display_dropdown" id="htid">
                <option selected value="<%=cstr(mrs("htid")) & "zooz" & mrs("htname") & "zooz" & mrs("hcolor")%>"><%=mrs("htid")%>.<%=mrs("htname")%>.<font color="<%=mrs("hcolor")%>">Home-Color(<%=mrs("hcolor")%>)</font> </option>
<%
...
%>			
          ...
          <tr> 
            ...
            <td><select name="atid" class="display_dropdown" id="atid">
                <option selected value="<%=cstr(mrs("atid")) & "zooz" & mrs("atname") & "zooz" & mrs("acolor")%>"><%=mrs("atid")%>.<%=mrs("atname")%>.<font color="<%=mrs("acolor")%>">Home-Color(<%=mrs("acolor")%>)</font> </option>
<%
tsql="select * from team where tid=0 "
for i=0 to ubound(ateam)
tsql=tsql&" or tid="&ateam(i)
