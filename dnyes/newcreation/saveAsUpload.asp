...
UploadSize=False
Response.Write"你要上传的文件大小超出程序限制,请<a href=dbm_index.asp>返回</a>修改重试"
Response.End
End If
...
Msg = ""
		...
		if ftitle<>"" and fileSize > 0 and UploadSize=True then
			...
			sfile = "./subsimg/"
			sql = "update dbm_subs set filetitle='"&ftitle&"',filedesc='"&filedesc&"',filetype='"&ftype&"',filecontenttype='"&file_ctype&"',sfile='"&sfile&filename&"',filepath='"&basepath&filename&"',filesize="&filesize&" where subs_id=" & myrequest("subs_id")
			conn.Execute(sql)
