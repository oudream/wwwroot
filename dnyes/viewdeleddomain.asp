              ...
              <tr> 
                ...
                <td width="100%"><br>
                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr valign="top"> 
                      <td colspan="2">+ 每天3-4位过期.com .net .org 域名公布 +</td>
                    </tr>
                    <tr valign="top"> 
...
%>
                    <tr> 
                      ...
                      <td width="493" valign="top"><a href="showdeledtable.asp?id=<%=rsnews("id")%>" title="<%=rsnews("deledtable")%>" target="_blank"> 
                        <% if len(rsnews("deledtable"))>100 then%>
                        <%=left(rsnews("deledtable"),98)%>.. 
                        <%else%>
                        <%=rsnews("deledtable")%> 
                        <%end if%>
                        </a></td>
...
%>
              ...
              <tr> 
                ...
                <td>&nbsp; <div align="right">共&nbsp;<%=amount%>&nbsp;个&nbsp;&nbsp;每页&nbsp;20&nbsp;个&nbsp;&nbsp;<font color=red><%=pagecount%></font>/<%=rsnews.pagecount%> 
                    ...
                    <a href="viewdeleddomain.asp?newstype=<%=zname%>&page=<%=cstr(pagecount+1)%>"><font color="#ff0000">下一页</font></a><font color="#ff0000"> 
                    <% end if %>
                    <% if rsnews.pagecount>1 and rsnews.pagecount=pagecount then %>
                    <a href="viewdeleddomain.asp?newstype=<%=zname%>&page=<%=cstr(pagecount-1)%>"> 
                    上一页</a> 
                    <%end if%>
                    <% if pagecount<>1 and rsnews.pagecount<>pagecount then%>
                    <a href="viewdeleddomain.asp?newstype=<%=zname%>&page=<%=cstr(pagecount-1)%>">上一页</a> 
                    <a href="viewdeleddomain.asp?newstype=<%=zname%>&page=<%=cstr(pagecount+1)%>"><font color="#ff0000">下一页</font></a><font color="#ff0000"> 
                    <% end if%>
                    &nbsp;</font></font></div></td>
              </tr>
