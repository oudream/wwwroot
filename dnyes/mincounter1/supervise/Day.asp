...
<%
	...
	var sSqlString = "select CountDate,HourCount,PageViewCount from v_Count "
		+ "where siteid="+iSiteId+" and (CountDate between #"+sLastMonth+"# and #"+sToday+"#) order by CountDate asc";
	var oRs = oConn.Execute( sSqlString );
	for(var i=0;i<iDayOfMonth;++i){
		var sTmpDate = new Date((1000*60*60*24) * i + dLastMonth.valueOf());
		if(!oRs.EOF){
	...
	var sNowFromTo = ' '+(sLastMonth)+' ~ '+(sToday)+'';
	...
	var sTotalFromTo = '';
	var sSqlString = "SELECT Day(CountDate),Sum(HourCount),Sum(PageViewCount) FROM v_Count"
		+ " WHERE siteid="+iSiteId
		+ " GROUP BY Day(CountDate)"
		+ " ORDER BY Day(CountDate) ASC";
	var oRs = oConn.Execute( sSqlString );
	for(var i=0;i<31;++i){
		if(!oRs.EOF){
    ...
    <td class="OuterHead"><table width="100%"  border="0" cellspacing="0" cellpadding="1">
      <tr>
        <td nowrap="nowrap" style="text-align:left">
			...
			<span id="ItemTitle"><font face="webdings">8</font> 月流量统计</span>
			<span class="SubHeader">&nbsp;
			<%="Scope: "+sNowFromTo+"  ("+iDayOfMonth+" Days), &nbsp; Total: "+iDaySum+" / "+iPageDaySum+" ."%>
			</span>
		</td>
  ...
  <tr>
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" class="InnerTable">
      <tr>
        ...
        <td class="InnerMain" title="[月访问情况一览]<%='\n'+sNowFromTo+'\n总访问量: '+iDaySum+' 次\n总浏览量: '+iPageDaySum+' 次'%>">
		<table width="100%"  border="0" cellspacing="0" cellpadding="0">
          <tr class="styTdBGLine">
					<%
						for(var i=0;i<aDay.length;++i){
							...
							Response.Write('<td class="TimeColumn" bgColor='+(aDay[aDay.length-1][0].getDate()==iDayOfMonth?(aDay[i][0].getDate()<11||aDay[i][0].getDate()>20?'fafafa':'white'):'white')+'>'
			...
			<% 
				for(var i=0;i<aDay.length;++i){
					if(aDay[i][0].getDay()<1||aDay[i][0].getDay()>5)
						Response.Write("<td align=center bgColor='#eeeeee'><a href='Hour.asp?QueryDate="+formatDateTime(aDay[i][0],2)+"'>"+(aDay[i][0].getDate())+"</a></td>\n");
					else
						Response.Write("<td align=center><a href='Hour.asp?QueryDate="+formatDateTime(aDay[i][0],2)+"'>"+(aDay[i][0].getDate())+"</a></td>\n");
				}
			%>
          </tr>
        </table></td>
          ...
          <tr class="MainText">
            <td height="26" align="center" valign="top">
						<input
						 ...
						 title="上一月" 
						 onFocus="blur()" 
						 onClick="location.href='<%=sUrl+'?QueryDate='+formatDateTime(new Date(new Date(dToday.getYear(),dToday.getMonth(),1)-(1000*60*60*24)),2)%>'"
						>
						</td>
          </tr>
          <tr class="MainText">
          ...
          <tr class="MainText">
            <td height="26" align="center" valign="bottom">
						<input
						 ...
						 title="下一月" 
						 onFocus="blur()" 
						 onClick="location.href='<%=sUrl+'?QueryDate='+formatDateTime(new Date(new Date(dToday.getYear(),dToday.getMonth()+2,1)-(1000*60*60*24)),2)%>'"
						>
						</td>
          </tr>
        </table></td>
...
<table width="760" border="0" cellpadding="2" cellspacing="1" class="OuterTable">
  <tr>
    <td class="OuterHead"><table width="100%"  border="0" cellspacing="0" cellpadding="1">
      <tr>
        <td nowrap="nowrap" style="text-align:left">
			<span id="ItemTitle"><font face="webdings">8</font> 所有31天访问情况一览</span>
			<span class="SubHeader">&nbsp;
			<%=""+"Total: "+iSumTotalHour+"/"+iPageSumTotalHour+"."%>
			</span>
		</td>
