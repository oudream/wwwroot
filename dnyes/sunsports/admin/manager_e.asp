...
function checkform()
{
	...
	if(! isNumber(form_administrator.default_score.value)) {
		alert('Use integers only for "Score" field');
		form_administrator.default_score.focus();
		return false;
		}
	if(! isNumber(form_administrator.default_yellow.value)) {
		alert('Use integers only for "Yellow Card" field');
		form_administrator.default_yellow.focus();
		return false;
		}
	if(! isNumber(form_administrator.default_red.value)) {
		alert('Use integers only for "Red Card" field');
		form_administrator.default_red.focus();
		return false;
		}
	...
	if(! isEnglish(form_administrator.uid.value)) {
		alert('Please enter a valid Userid. ');
		form_administrator.uid.focus();
		return false;
		}
...
conn.Execute("update player set name='"&uname&"',tel='"&tel&"',zip='"&zip&"',contact='"&contact&"',email='"&email&"',nric='"&nric&"',uid='"&uid&"',pwd='"&pwd&"',jersey="&jersey&",default_score="&default_score&",default_yellow="&default_yellow&",default_red="&default_red&" where pid=" & pid)
conn.Execute("update announcements set name='"&uname&"' where pid=" & pid)
conn.Execute("update card set pname='"&uname&"' where pid=" & pid)
conn.Execute("update scorer set pname='"&uname&"' where pid=" & pid)

if page="" or adminlevel="" then
url=zurl&"?tid="&tid&"&option=editmanagersucc"
else
url="user_v.asp?page=" & page & "&adminlevel=" & adminlevel & "&option=editmanagersucc"
end if
...
<H2><FONT face="Verdana, Arial, Helvetica, sans-serif" 
                  color=#000066><strong>Edit Manager</strong></FONT></H2>
<!----------------------------------------------------------------------------------------------------------------------
<!--                                                   ERROR MSGS
<!-------------------------------------------------------------------------------------------------------------------- -->
<%
...
%>
<FORM action="manager_e.asp" method="post" name="form_administrator" id="form_administrator" onSubmit="return checkform();">
...
<!------------------------------------------------------------------------------------------------------------------------------------ -->
      ...
      <TR> 
        <TD height=43> <DIV align=center><FONT 
                        face="Verdana, Arial, Helvetica, sans-serif" 
                        color=#000000 size=3>Score</FONT></DIV></TD>
        <TD><FONT face="Verdana, Arial, Helvetica, sans-serif" 
                        color=#000000 size=3> 
          <input maxlength=3 size=5 id="default_score" name="default_score" value=<%=prs("default_score")%>>
          </FONT></TD>
      </TR>
      <TR> 
        <TD height=43> <DIV align=center><font color="#000000" size="3" face="Verdana, Arial, Helvetica, sans-serif">Yellow 
            Card </font></DIV></TD>
        <TD><FONT face="Verdana, Arial, Helvetica, sans-serif" 
                        color=#000000 size=3> 
          <input maxlength=3 size=5 id="default_yellow" name="default_yellow" value=<%=prs("default_yellow")%>>
          </FONT></TD>
      </TR>
      <TR> 
        <TD height=43> <DIV align=center><FONT 
                        face="Verdana, Arial, Helvetica, sans-serif" 
                        color=#000000 size=3>Red Card</FONT></DIV></TD>
        <TD><FONT face="Verdana, Arial, Helvetica, sans-serif" 
                        color=#000000 size=3> 
          <input maxlength=3 size=5 id="default_red" name="default_red" value=<%=prs("default_red")%>>
          </FONT></TD>
      </TR>
      <TR> 
      ...
      <TR> 
        <TD colSpan=2> <P><FONT face="Verdana, Arial, Helvetica, sans-serif" 
          ...
          <P><STRONG><FONT 
            ...
            <INPUT type="hidden" id="adminlevel" name="adminlevel" value=<%=adminlevel%>>
            ...
            <br>
            <br>
            <font color="#FF0000">#</font> Because one team must has one manager 
