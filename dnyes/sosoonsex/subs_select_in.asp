...
function gettarg(targ,selObj,restore){ 
  eval(targ+".location='subs_select_in.asp?option="+'取消选择'+"&sort_id="+selObj.options[selObj.selectedIndex].value+"'");
  if (restore) selObj.selectedIndex=0;
}
function getbrand(targ,selObj,restore,sort_id){ 
  eval(targ+".location='subs_select_in.asp?option="+'取消选择'+"&sort_id="+sort_id+"&brand_id="+selObj.options[selObj.selectedIndex].value+"'");
  if (restore) selObj.selectedIndex=0;
}
function getsubs(targ,selObj,restore,sort_id,brand_id){ 
  eval(targ+".location='subs_select_in.asp?option="+'取消选择'+"&sort_id="+sort_id+"&subs_id="+selObj.options[selObj.selectedIndex].value+"&brand_id="+brand_id+"'");
  if (restore) selObj.selectedIndex=0;
}
function checkform()
{
...
%>
<form name="form_administrator" method="post" action="subs_select_in.asp" onSubmit="return checkform();">
  <table width="97%" border="1" align="center" cellpadding="0" cellspacing="0" bordercolor="#EBEBEB">
    <tr> 
      <td width="100" align="center" bgcolor="#FFFFFF"> <select name="sort_id" id="select" class="display_dropdown"  onChange="gettarg('self',this,0)">
          <%
...
%>
	  ...
	  <select name="brand_id" id="select2" class="display_dropdown"  onChange="getbrand('self',this,0,<%=sort_id%>)">
          <%
if brand_id=0 then
response.Write("<option value='' selected>请选择……</option>")
else
