...
conn.Execute("update subspass set gid="&gid&",gcontacter='"&gcontacter&"',pyear="&pyear&",pmonth="&pmonth&",pday="&pday&",subs_id="&subs_id&",mount="&mount&",price="&price&",counter="&counter&",memo='"&memo&"',operationer_id="&operationer_id&",user_id="&user_id&" where subspass_id=" & subspass_id )
...
%>
      ...
      <TR align="center"> 
        <TD height="50" colSpan=2><table width="100%" border="1" cellspacing="0" cellpadding="0">
            <tr align="center"> 
              <td width="200" height="30">商品名称及型号/规格 (单位)</td>
              <td width="80">数量</td>
              <td width="80">单价</td>
              <td width="80">金额</td>
              <td width="130">备注</td>
...
%>
                <option value=<%=srs("subs_id")%> selected><%=brs("bname")&" "&srs("code")&" "%> ( <%=srs("suit")%> )</option>
                <%
end if
brs.close
end if
...
if not ( brs.eof or brs.bof ) then
%>
                <option value=<%=srs("subs_id")%>><%=brs("bname")&" "&srs("code")&" "%> ( <%=srs("suit")%> )</option>
                <%
end if
brs.close
srs.movenext
