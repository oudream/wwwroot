...
function about()
{
window.showModalDialog("about.htm","","dialogWidth:420px;dialogHeight:310px;scroll:no;status:no;help:no");
}
...
%>
	...
	<td colspan="2" height="25" class="unnamed2" valign="middle"  class="TDtop">
		<div align="center" >┊ <B>在<b class="unnamed2">[<a href="BigClassManage.asp?TypeID=<%=rs2("typeid")%>"><%=typename%></a>]-[<a href="SmallClassManage.asp?BigClassID=<%=rs1("bigclassid")%>"><%=bigclassname%></a>]-[<a href="ListNews.asp?SmallClassID=<%=smallclassid%>"><%=smallclassname%></a>]</b>中添 加 文 章</B> ┊</div>
	</td>
<%rs3.close
set rs3=nothing
rs2.close
...
%>
	...
	<td colspan="2"  height="29" class="unnamed2" valign="middle" bgcolor="<%=m_top%>" width="100%">在<b class="unnamed2">
		<a href="BigClassManage.asp?typeid=<%=rs2("typeid")%>"><%=typename%></a>-<a href="SmallClassManage.asp?bigclassid=<%=rs1("bigclassid")%>"><%=bigclassname%></a></b>中添 加 文 章
	</td>
<%rs2.close
set rs2=nothing
rs1.close
...
<!--增加引用样式结束-->
		...
		<br>&nbsp;&nbsp;<iframe style="top:2px" ID="UploadFiles" src="upme.htm" frameborder=0 scrolling=no width="280" height="25"></iframe></div>
               <script language="javascript">
                var bTextMode=false;
		    	document.write ('<iframe src="textbox.asp?action=new" id="message" width="97%" height="200" align=center></iframe>')
              ...
              <tr> 
                ...
                <td bgcolor="#FFFFFF">&nbsp;&nbsp;<input name="PicUrl" type="text" id="PicUrl" size="30" maxlength="200" style="background-color:ffffff;border: 1 double">
