...
if not isempty(request("ReViewID")) and IsNumeric(request("ReViewID")) then
	sql="Select * From "& db_Review_Table &" where ReViewID="& CheckStr(trim(request("ReViewID"))) &" Order by ReviewId DESC"
else
	sql="Select * From "& db_Review_Table &" where NewsId is NULL or NewsId < 1 Order by ReviewId DESC"
end if
rs.open sql,conn,1,1
if rs.eof and rs.bof then
	if not isempty(request("ReViewID")) and IsNumeric(request("ReViewID")) then
		response.write "<br><br><br><p align=center><font color=red>找 不 到 你 指 定 的 留 言</font></p><br><br>"
	else
								...
								<td valign="top">
									<table width="100%" border="0" cellspacing="0" cellpadding="0" >
										<tr>
											...
											<%end if%>
											<%if rs("homepage")<>"" then%>
												<a href="<%=rs("homepage")%>" target="_blank"><img src="images/homepage.gif" border="0" title='主页'></a>
											<%else%>
												<img src="images/homepage.gif" border="0">
											<%end if%>
											<% if rs("oicq")<>"" then%>
												<a href="http://search.tencent.com/cgi-bin/friend/user_show_info?ln=<%=rs("oicq")%>" ><img src="images/OICQ.gif" border="0"  title='显示QQ信息'></a>
											<%else%>
												<img src="images/OICQ.gif" border="0">
											<%end if%>
											
											    <%if aaas=1 then %>
												<a href="GuestEdit.asp?ReviewId=<%=rs("reviewid")%>"><img src="images/EDIT.GIF" border="0" title='编辑'></a> 
												<a href="guestreply.asp?reviewid=<%=rs("reviewid")%>"><img src="images/reply.GIF" border="0" title='回复'></a> 
												<a href='CheckReView.asp?ReviewID=<%=rs("ReviewID")%>&Guest=2'><%if rs("passed")=0 then%><img src="images/pass0.GIF" border="0" title='通过审核'><%else%><img src="images/pass1.GIF" border="0" title='取消审核'><%end if%></a>
									...
									<table width="98%" border="0" cellspacing="0" cellpadding="0" style="table-layout:fixed; word-break:break-all;border-collapse: collapse">
										<tr>
											...
											<%if rs("passed")=1 then
												...
												response.write "<center><BR><font color=red><B>留言未通过审核</B></font></center>"
												if aaas=1 then
													response.write "<font color=green>以下内容请审查核定：</font><br><br>"
													response.write rs("content")
												end if
											end if%>
												</p>
