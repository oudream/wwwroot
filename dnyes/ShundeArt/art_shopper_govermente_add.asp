...
response.Write("<SCRIPT LANGUAGE=JavaScript> alert('出错！此地方政府买家有同名存在，请用其它名字输入');</SCRIPT>")
urs.close
set urs=nothing
else
urs.close
...
if brief="" then brief=" "
if memo="" then memo=" "
sql="insert into art_shopper_govermente (allname,addr,tel,fax,zip,email,web_addr,contacter,brief,memo,inserttime) values ('"&allname&"','"&addr&"','"&tel&"','"&fax&"','"&zip&"','"&email&"','"&web_addr&"','"&contacter&"','"&brief&"','"&memo&"','"&now()&"')"
conn.Execute(sql)
...
<TABLE width=100% 
  ...
  <FORM action="art_shopper_govermente_add.asp" method="post" name="form_administrator" id="form_administrator" onSubmit="return checkform();">
    <TBODY>
      <TR align="center"> 
        <TD colspan="2"><font color="#FF0000" size="+1"><strong>地方政府买家添加</strong></font></TD>
      </TR>
      <TR> 
