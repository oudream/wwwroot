...
conn.execute "delete * from art_collecter_society where art_collecter_society_id=" & request("art_collecter_society_id")
...
<form name="form_administrator" id="form_administrator" method="post" action="art_collecter_society_view.asp">
...
sql="select * from art_collecter_society where allname<>'' " 
...
sql = sql &" order by art_collecter_society_id DESC"
...
<table width="100%" border="1" align="center" cellpadding="2" cellspacing="0" bordercolor="#CCCCCC">
  <tr bgcolor="#FFFFFF"> 
    <td height="22" colspan="8" align="center" class="header"><font color="#FF0000" size="+1"><strong>美术家协会列表</strong></font></td>
  </tr>
  <tr align="center"> 
    ...
    <td width="100" bgcolor="#FFFFFF">会长</td>
...
%>
  <tr align="center"> 
    <td width="100" height="25" bgcolor="#FFFFFF"><a href="art_collecter_society_edit.asp?art_collecter_society_id=<%=rs("art_collecter_society_id")%>"><%=rs("allname")%></a>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("addr")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("tel")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("fax")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("cap")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><a href="art_collecter_society_viewall.asp?art_collecter_society_id=<%=rs("art_collecter_society_id")%>" target="_blank">详细</a>&nbsp; | &nbsp;<a href="art_collecter_society_edit.asp?art_collecter_society_id=<%=rs("art_collecter_society_id")%>">编辑</a>&nbsp; | &nbsp;<a href="art_collecter_society_view.asp?art_collecter_society_id=<%=rs("art_collecter_society_id")%>&option=del" onClick="return confirm('确定删除此项吗？')">删除</a>&nbsp;</td>
  </tr>
  <%
rs.movenext
i=i+1                                                                     
...
if zj<=zk*15 then
%>
          <td><a href="art_collecter_society_view.asp?zname=<%=zname%>&zaddr=<%=zaddr%>&ztel=<%=ztel%>&zfax=<%=zfax%>&zcap=<%=zcap%>&zothers=<%=zothers%>&page=<%=zj%>">&nbsp;<%=zj%>&nbsp;</a></td>
          <%
else
zk=zk+1
%>
