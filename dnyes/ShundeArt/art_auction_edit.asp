...
if brief="" then brief=" "
if memo="" then memo=" "
conn.Execute("update art_auction set allname='"&allname&"',classes='"&classes&"',addr='"&addr&"',tel='"&tel&"',fax='"&fax&"',zip='"&zip&"',email='"&email&"',web_addr='"&web_addr&"',creation_date='"&creation_date&"',legal_representative='"&legal_representative&"',contacter='"&contacter&"',ctel='"&ctel&"',brief='"&brief&"',memo='"&memo&"' where art_auction_id=" & request("art_auction_id"))
...
<FORM action="art_auction_edit.asp" method="post" name="form_administrator" id="form_administrator" onSubmit="return checkform();">
    <TBODY>
      <TR align="center"> 
        <TD height="30" colspan="2"><font color="#FF0000" size="+1"><strong>拍卖行编缉</strong></font> 
        </TD>
      </TR>
      <TR> 
      ...
      <TR> 
        ...
        <TD> <select name="classes" id="classes">
            ...
            <option value="<%=rs("classes")%>" selected><%=rs("classes")%></option>
      ...
      <TR> 
        ...
        <TD> <INPUT maxLength=40 size=30 id="legal_representative" name="legal_representative" value="<%=rs("legal_representative")%>"> 
        </TD>
      </TR>
      <TR> 
        <TD align="right"> 联系人：</TD>
        <TD> <INPUT maxLength=40 size=30 id="contacter" name="contacter" value="<%=rs("contacter")%>"> 
        </TD>
      </TR>
      <TR> 
        <TD align="right"> 联系人电话：</TD>
        <TD> <INPUT maxLength=100 size=30 id="ctel" name="ctel" value="<%=rs("ctel")%>"> 
        </TD>
      </TR>
      <TR> 
      ...
      <TR align="center"> 
        <TD height="50" colSpan=2> <INPUT type=submit id="option" name="option" value="edit"> 
          ...
          <input type="hidden" id="art_auction_id" name="art_auction_id" value="<%=request("art_auction_id")%>"> 
        </TD>
      </TR>
    </TBODY>
