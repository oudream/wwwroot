...
Const db_Board_Table = "[board1]"		'公告板信息表X
...
Const db_Vote_Table = "[vote1]"			'投票信息表
Const db_Manage_Table = "[User]"		'后台配置管理用户表
'======================================================================
'新闻系统与被整合系统共用的用户数据表
Const db_User_Table = "[User]"		'与整合系统共用的用户表
						'动网6.1 为 [User]
						'动网7.0 为 [Dv_User]
...
Const db_User_Question = "Quesion"		'question	用户问题
Const db_User_Answer = "Answer"			'answer	用户回答
Const db_User_Face = "Face"			'photo		用户照片文件路径
Const db_User_RegDate = "AddDate"		'dateandtime	用户注册时间
...
Const db_User_FaceWidth = "Width"		'用户头象宽度
Const db_User_FaceHeight = "Height"		'用户头象高度

''原大奔整合Dvbbs 6.10 版数据库，如要分开统计用户在图文系统与BBS上的发文章数量，
''请在 USER 表中添加 number 字段和个人详细信息 content 字段,字段属性请参照原图文系统。
...
UserTableType = "Dvbbs"	    			'整合动网论坛则值为"Dvbbs"
						'不整合则值为"FeiTium"
						'---------------------------------
...
eWebEditor="0"					'是否启用eWebEditor来代替原编辑器，值为"1"时启用，其它值为不启用。
...
if UserTableType = "FeiTium" then
	...
	if UserTableType = "Dvbbs" then
		'新闻系统 V0.45Finish版与动网论坛共用数据库的BBS设置参数表
		...
		FaceDefault = "Images/userface/image1.gif"
							'用户注册（图文侧）默认头象文件，相对于 BbsPath 路径。
	end if
end if
