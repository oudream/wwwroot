...
usql="select * from art_artist_society where allname='"&allname&"' and art_artist_society_id<>" & request("art_artist_society_id")
set urs=server.createobject("ADODB.Recordset")
urs.open usql,conn,1,1
if not ( urs.eof or urs.bof ) then
response.Write("<SCRIPT LANGUAGE=JavaScript> alert('出错！此美术家协会有同名存在，请用其它名字输入');</SCRIPT>")
urs.close
set urs=nothing
else
urs.close
...
if brief="" then brief=" "
if memo="" then memo=" "
conn.Execute("update art_artist_society set allname='"&allname&"',addr='"&addr&"',tel='"&tel&"',fax='"&fax&"',zip='"&zip&"',email='"&email&"',web_addr='"&web_addr&"',creation_date='"&creation_date&"',cap='"&cap&"',brief='"&brief&"',memo='"&memo&"' where art_artist_society_id=" & request("art_artist_society_id"))
response.Write("<SCRIPT LANGUAGE=JavaScript> alert('编辑成功');</SCRIPT>")
end if
end if
%>
<%
sql="select * from art_artist_society where art_artist_society_id="&request("art_artist_society_id") 
...
<FORM action="art_artist_society_edit.asp" method="post" name="form_administrator" id="form_administrator" onSubmit="return checkform();">
    <TBODY>
      <TR align="center"> 
        <TD height="30" colspan="2"><font color="#FF0000" size="+1"><strong>美术家协会编缉</strong></font></TD>
      </TR>
      <TR> 
      ...
      <TR> 
        ...
        <TD> <INPUT maxLength=40 size=30 id="creation_date" name="creation_date" value="<%=rs("creation_date")%>"> 
        </TD>
      </TR>
      <TR> 
        <TD align="right"> 会长：</TD>
        <TD> <INPUT maxLength=40 size=30 id="cap" name="cap" value="<%=rs("cap")%>"> 
        </TD>
      </TR>
      <TR> 
      ...
      <TR align="center"> 
        <TD height="50" colSpan=2> <INPUT type=submit id="option" name="option" value="edit"> 
          ...
          <input type="hidden" id="art_artist_society_id" name="art_artist_society_id" value="<%=request("art_artist_society_id")%>"> 
        </TD>
      </TR>
    </TBODY>
