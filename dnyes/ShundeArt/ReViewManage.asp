...
if not(request.cookies(Forcast_SN)("ManageKEY")="super" or request.cookies(Forcast_SN)("ManageKEY")="typemaster" or request.cookies(Forcast_SN)("ManageKEY")="bigmaster") then
	...
	if reviewcheckshow="1" then
		rs.Source="select * from "& db_Review_Table &" where newsid is not null and newsid > 0 order by reviewID desc"
	else
		rs.Source="select * from "& db_Review_Table &" where passed=0 and newsid is not null newsid > 0 order by reviewID desc"
	end if
...
<tr class="TDtop">
	<td colspan="8" height=25 class="TDtop" align="center">┊ <B>审核评论</B> ┊ </td>
</tr>
<tr>
...
<tr bgcolor=ffffff>
	...
	<td align=center><a href='ReadNews.asp?NewsID=<%=rs("NewsID")%>' target="_blank"><%=rs("NewsID")%></a></td>
	<td align="left"><a href='ReadView.asp?ReViewID=<%=rs("ReViewID")%>' target=_blank><%=htmlencode(DisplayContent)%>...</a></td>
