...
conn.execute "delete * from art_auction where art_auction_id=" & request("art_auction_id")
...
<form name="form_administrator" id="form_administrator" method="post" action="art_auction_view.asp">
...
sql="select * from art_auction where allname<>'' " 
...
sql = sql &" order by art_auction_id DESC"
...
<table width="100%" border="1" align="center" cellpadding="2" cellspacing="0" bordercolor="#CCCCCC">
  <tr bgcolor="#FFFFFF"> 
    <td height="30" colspan="8" align="center" class="header"><font color="#FF0000" size="+1"><strong>拍卖行列表</strong></font></td>
  </tr>
  <tr align="center"> 
...
%>
  <tr align="center"> 
    <td height="25" bgcolor="#FFFFFF"><a href="art_auction_edit.asp?art_auction_id=<%=rs("art_auction_id")%>"><%=rs("allname")%></a>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("addr")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("tel")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("fax")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><%=rs("contacter")%>&nbsp;</td>
    <td bgcolor="#FFFFFF"><a href="art_auction_viewall.asp?art_auction_id=<%=rs("art_auction_id")%>" target="_blank">详细</a>&nbsp; | &nbsp;<a href="art_auction_edit.asp?art_auction_id=<%=rs("art_auction_id")%>">编辑</a>&nbsp; | &nbsp;<a href="art_auction_view.asp?art_auction_id=<%=rs("art_auction_id")%>&option=del" onClick="return confirm('确定删除此项吗？')">删除</a>&nbsp;</td>
  </tr>
  <%
rs.movenext
i=i+1                                                                     
...
if zj<=zk*15 then
%>
          <td><a href="art_auction_view.asp?zname=<%=zname%>&zaddr=<%=zaddr%>&ztel=<%=ztel%>&zfax=<%=zfax%>&zcontacter=<%=zcontacter%>&zothers=<%=zothers%>&page=<%=zj%>">&nbsp;<%=zj%>&nbsp;</a></td>
          <%
else
zk=zk+1
%>
