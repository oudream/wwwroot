...
if Cint(Forum_Setting(36)) = 1 or PostUserGroup<4 then
	...
	strContent= re.Replace(strContent,"<br><IMG SRC="""&Forum_info(7)&"$1.gif"" border=0>此主题相关图片如下：<br><A HREF=""$2$1"" TARGET=_blank><IMG SRC=""$2$1"" border=0 alt=按此在新窗口浏览图片 onload=""javascript:if(this.width>screen.width-333)this.width=screen.width-333""></A>")
end if

re.Pattern="\[UPLOAD=(\S+?)\](.+?)\[\/UPLOAD\]"
strContent= re.Replace(strContent,"<br><IMG SRC="""&Forum_info(7)&"$1.gif"" border=0> <a href=""$2"">点击浏览该文件</a>")
