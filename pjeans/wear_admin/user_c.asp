...
usql="select * from admin_users where uid='"&uid&"'"
set urs=server.createobject("ADODB.Recordset")
urs.open usql,conn,1,1
if not ( urs.eof or urs.bof ) then
response.Write("<SCRIPT LANGUAGE=JavaScript> alert(' ERROR !!! The Userid have existed , please enter other Userid . ');</SCRIPT>")
urs.close
set urs=nothing
else
urs.close
...
if email="" then email=" "
if nric="" then nric=" "
sql="insert into admin_users (name,tel,zip,contact,email,uid,pwd,nric,adminlevel,inserttime) values ('"&uname&"','"&tel&"','"&zip&"','"&contact&"','"&email&"','"&uid&"','"&pwd&"','"&nric&"',"&adminlevel&",'"&now()&"')"
conn.Execute(sql)
