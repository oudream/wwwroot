...
ourefer=request("hrefer")

tsql="select * from rresult where mid="&mmid&" and tid="&tid
Set trs=Server.CreateObject("ADODB.RecordSet") 
trs.Open tsql,conn,1,1 
if trs.eof or trs.bof then
conn.Execute("insert into rresult (mid,tid,hscore,ascore,refer) values ("&mmid&","&tid&","&hhacountscore&","&aacountscore&","&ourefer&")")
else
conn.Execute( "update rresult set hscore="&hhacountscore&", refer="&ourefer&", ascore="&aacountscore&" where mid="&mmid&" and tid="&tid )
end if
trs.close
set trs=nothing
...
trs.Open tsql,conn,1,1 
if trs.eof or trs.bof then
conn.Execute("insert into rresult (mid,tid,hscore,ascore,refer) values ("&mmid&","&htid&","&hhacountscore&","&aacountscore&","&ourefer&")")
else
conn.Execute( "update rresult set hscore="&hhacountscore&", refer="&ourefer&", ascore="&aacountscore&" where mid="&mmid&" and tid="&htid )
end if
trs.close
set trs=nothing

response.Redirect("match_r_edeff.asp?option=editresultissucc&mid="&mmid)
end if
%>
